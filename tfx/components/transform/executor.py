...
class Executor(base_executor.BaseExecutor):
        ...
        if compute_statistics or materialization_format is not None:
          ...
          if (compute_statistics and
            ...
            if use_tfxio and self._IsDataFormatSequenceExample(
                raw_examples_data_format):
              def _ExtractRawExamples(record_batch):
              ...
              for dataset in analyze_data_list:
                ...
                stats_input.append(
                    dataset.standardized
                    | 'ExtractRawExamples[{}]'.format(infix) >> beam.Map(
                        _ExtractRawExamples)
                    | 'DecodeSequenceExamplesAsExamplesIntoRecordBatches[{}]'
                    .format(infix) >> beam.ParDo(
                        Executor._ToArrowRecordBatchesFn(schema_proto)))
            else:
              stats_input = [
  ...
  def _GetTFXIOPassthroughKeys(self) -> Optional[Set[Text]]:
    """Always returns None."""
    return None
